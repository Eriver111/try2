...
function analyzeCharacter(bazi) {
    ...
    const ganNature = {
        '甲': {
            ...
            negative: '可能过于耿直不懂变通，略显固执强势，容易承担过多压力，有时刚愎自用。',
            xingxiang: '甲木如松柏栋梁，挺拔不屈。天性向上生长，内心有一股不服输的劲头。待人真诚但有原则底线，不喜拐弯抹角。'
        },
        '乙': {
            positive: '温和细腻、善解人意，适应力强、善于交际，如藤蔓般柔韧灵活，能在复杂环境中找到生存空间。',
            negative: '有时过于敏感依赖他人，内心占有欲较强，可能在关键时刻缺乏决断力。',
            xingxiang: '乙木如花草藤萝，看似柔弱实则坚韧。心思细密、观察入微，善于借势而为。表面随和，内心却有自己的一套逻辑和坚持。'
        },
        '丙': {
            positive: '热情大方、光明磊落，有强烈的感染力和号召力，如太阳般温暖周围，乐于助人、慷慨仗义。',
            negative: '偶尔冲动急躁、好面子、霸道强势，情绪外放时可能忽略他人感受。',
            xingxiang: '丙火如烈日当空，热情奔放。天生自带光芒，走到哪里都是焦点。直来直往、不藏心机，但也因此容易得罪人而不自知。'
        },
        '丁': {
            positive: '心思细腻、富有理性，外柔内明，如烛火般持续温暖，善于洞察人心却不张扬，忠诚可靠。',
            negative: '有时多疑敏感、缺乏自信，容易陷入自我怀疑。表面顺从内心倔强。',
            xingxiang: '丁火如灯烛炉火，柔而有光。不似丙火那般耀眼，却更加持久温暖。内在洞察力极强，是很好的倾听者和参谋。'
        },
        '戊': {
            positive: '诚实守信、稳重务实，如大地般包容万象，有耐心、有毅力，是值得托付的朋友和伙伴。',
            negative: '有时显得保守迟钝、缺乏灵活性，对外界变化反应较慢，可能过于墨守成规。',
            xingxiang: '戊土如城墙山峦，厚重沉稳。不轻易表态，但一旦承诺必全力以赴。内心有强烈的责任感，是团队中最可靠的基石。'
        },
        '己': {
            positive: '温和宽厚、善良包容，处事圆融有分寸，善于协调人际关系，如田园沃土般滋养万物。',
            negative: '有时优柔寡断、过于迁就，可能在原则问题上退让过多，内心容易纠结。',
            xingxiang: '己土如田园土壤，温和滋养。待人真诚、重情重义，有很强的共情能力。不喜冲突，宁可自己吃亏也不愿让别人难堪。'
        },
        '庚': {
            positive: '刚毅果断、正义感强，执行力一流，如刀剑般锐利有力，不畏艰难、迎难而上，是天生的实干家。',
            negative: '有时显得冷酷无情、好斗偏激，缺乏柔性变通，可能在人际关系中过于刚硬。',
            xingxiang: '庚金如刀斧利器，锋芒毕露。行动力极强、说干就干，讨厌拖延和妥协。内心有强烈的正义标准，黑白分明、不搞中庸。'
        },
        '辛': {
            positive: '精致细腻、品味高雅，如珠宝般追求完美，有独特的审美和艺术气质，做事一丝不苟。',
            negative: '可能过于挑剔刻薄、对人对己要求太高，容易陷入完美主义的困境，不易满足。',
            xingxiang: '辛金如珠玉首饰，精致玲珑。天生的完美主义者，对细节有近乎偏执的追求。内心骄傲而敏感，渴望被认可和欣赏。'
        },
        '壬': {
            positive: '智慧圆融、胸怀宽广，如江河般通达变通，善于学习、乐于分享，是天生的智者和沟通者。',
            negative: '有时太过随性散漫、不够专注执着，可能在关键时候缺乏定力，容易三分钟热度。',
            xingxiang: '壬水如江河奔流，灵活多变。思维敏捷、反应快、适应力极强。喜欢自由、讨厌束缚，是天生的多面手但需要找到聚焦的方向。'
        },
        '癸': {
            positive: '聪慧细腻、直觉敏锐，如雨露般润物无声，有很强的感知力和创造力，内心世界丰富而深邃。',
            negative: '有时过于敏感多疑、情绪化，容易陷入幻想而不切实际，对批评反应强烈。',
            xingxiang: '癸水如晨露甘霖，温柔细腻。外表安静内心波澜，有极强的直觉和洞察力。善于思考、不喜张扬，是天生的策划者和战略家。'
        }
    };
    ...
    pillars.forEach(pos => {
        const ganSS = getShiShen(DAY, bazi[pos].gan);
        ssCount[ganSS] = (ssCount[ganSS] || 0) + 1;
        getCangGan(bazi[pos].zhi).forEach(g => {
            const ss = getShiShen(DAY, g);
            ssCount[ss] = (ssCount[ss] || 0) + 1;
        });
    });

    // 排序取前3
    const ssSorted = Object.entries(ssCount).sort((a,b) => b[1] - a[1]);
    const top3 = ssSorted.slice(0, 3);

    // 最旺十神性格影响
    const ssTraits = {
        '正官': '责任感强、重视规则和社会形象，做事有分寸，善于规划和管理，是「靠谱」的代名词。',
        '七杀': '竞争心强、敢闯敢拼，有强烈的进取精神和不服输的个性，是天生的创业者或领导者。',
        '正印': '心地善良、喜欢学习、乐于助人，重视精神世界的富足，有一种天然的慈悲和包容。',
        '偏印': '思维独特、有创意有灵气，不喜随大流，可能在专业领域有独到见解，适合做幕后智囊。',
        '正财': '务实稳重、善于理财，对物质生活有规划，做事脚踏实地，不喜欢风险和不确定性。',
        '偏财': '大方豪爽、有商业头脑，敢投资敢冒险，对金钱和机会有敏锐的嗅觉，社交场合如鱼得水。',
        '食神': '温和乐观、有艺术才华，喜欢舒适安逸的生活，心态平和、会享受，是身边人的开心果。',
        '伤官': '才华横溢、个性鲜明，有强烈的表现欲和创造力，不愿被束缚，追求精神自由和独立。',
        '比肩': '刚强独立、自主意识强，不喜受人管束，与人平等相处，重视朋友和兄弟情谊。',
        '劫财': '有竞争意识、精力旺盛，行动力强但不喜按规矩出牌，在群体中有很强的影响力。'
    };

    const topSSNames = top3.map(t => t[0]);

    // 生成综合性格概述
    const nature = ganNature[DAY];
    let composite = nature.xingxiang + ' ';

    if (topSSNames.length >= 1) {
        const mainSS = topSSNames[0];
        composite += '命局中' + mainSS + '星最旺（出现' + top3[0][1] + '次），体现为' + ssTraits[mainSS];
    }
    if (topSSNames.length >= 2) {
        composite += '，同时' + topSSNames[1] + '星为辅，彰显了更多元的性格维度。';
    }
    composite += ' 综合来看，' + nature.positive + ' 但需留意，' + nature.negative;
