...
;(function() {
  ...
  function renderAll(result, relationType, p1, p2) {
    ...
    var html = '';
    html += renderXiyong(result.xiyong, p1, p2);
    html += renderWuxingComplement(result.wuxingComplement, p1, p2);
    html += renderDailyRelation(result.dailyRelation, p1, p2);
    html += renderDayGanStrength(result.dayGanStrength, p1, p2);
    html += renderCrossPillars(result.crossPillars);
    html += renderCoreMode(result.coreMode);
    html += renderYearlyAdvice(result.yearlyAdvice);
    html += renderDosAndDonts(result.dosAndDonts);
  ...
  // 构建 drawer 包装器
  function drawer(title, bodyHtml, openByDefault) {
    ...
    return '<div class="' + cls + '">' +
      ...
      '</div>' +
      '<div class="hp-drawer-body">' + bodyHtml + '</div>' +
    '</div>';
  }
  ...
  function renderXiyong(xiyongData, p1, p2) {
    if (!xiyongData || !xiyongData.p1 || !xiyongData.p2) {
      return drawer('喜用神分析', '<p>数据暂不可用</p>', false);
    }
    ...
    return drawer('喜用神分析',
      ...
      '</div>' +
      (detail ? '<div class="hp-xiyong-complement">' + escapeHtml(detail) + '</div>' : ''), true);
  }
  ...
  function renderWuxingComplement(wcData, p1, p2) {
    if (!wcData) {
      return drawer('五行互补', '<p>数据暂不可用</p>', false);
    }
    ...
    return drawer('五行互补',
      ...
      '<div class="hp-wuxing-score">互补度评分：' + (wcData.complementScore || 0) + ' 分</div>', true);
  }
  ...
  function renderDailyRelation(drData, p1, p2) {
    if (!drData) {
      return drawer('日柱契合度', '<p>数据暂不可用</p>', false);
    }

    return drawer('日柱契合度',
      '<div class="hp-daily-score">契合评分：' + (drData.score || 0) + ' / 60</div>' +
      '<div class="hp-daily-desc">' + escapeHtml(drData.ganDesc || '') + '</div>' +
      '<div class="hp-daily-desc">' + escapeHtml(drData.zhiDesc || '') + '</div>', true);
  }
  ...
  function renderDayGanStrength(dgsData, p1, p2) {
    if (!dgsData) {
      return drawer('日干旺衰对比', '<p>数据暂不可用</p>', false);
    }
    ...
    return drawer('日干旺衰对比',
      ...
      '<div class="hp-dgs-detail">' + escapeHtml(dgsData.detail || '') + '</div>', true);
  }
  ...
  function renderCrossPillars(crossData) {
    if (!crossData || !crossData.length) {
      return drawer('跨盘干支交互',
        '<p class="hp-no-data">两盘之间未发现明显干支交互关系。</p>', false);
    }
    ...
    return drawer('跨盘干支交互',
      '<div class="hp-cross-list">' + itemsHtml + '</div>', true);
  }
  ...
  function renderCoreMode(coreMode) {
    if (!coreMode) {
      return drawer('核心相处模式', '<p>数据暂不可用</p>', false);
    }

    return drawer('核心相处模式',
  ...
  function renderYearlyAdvice(yearlyData) {
    if (!yearlyData || !yearlyData.length) {
      return drawer('流年关键节点', '<p>数据暂不可用</p>', false);
    }
    ...
    return drawer('流年关键节点',
      '<div class="hp-yearly-grid">' + cardsHtml + '</div>', true);
  }
  ...
  function renderDosAndDonts(ddData) {
    if (!ddData) {
      return drawer('宜忌清单', '<p>数据暂不可用</p>', false);
    }
